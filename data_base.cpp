...
{
    /** \brief Loads file into internal string buffer line by line*/
    /*std::string temp = "";
    if (file.is_open())
    {
        while ( file.good() )
...
{
    /** \brief This function takes a search string (aka. keyword) and looks for the index of the beginning of the keyword.
...
{
    /** \brief Extracts the string in much the same way the GetValueFromData does.*/
    int start = findString((char*)search.c_str(), buffer.c_str())+ search.length();
...
/** \brief This method opens a file in either read or truncated write mode. The write mode might clear file of previous contents
 *
 * \param const char* location
 * \param bool readMode, default value is true
 * \return nothing
 *
 */
{
...
{
    /** \brief Will open the output stream in append mode. It assumes you just want to ad some new stuff to the file immediately.*/
...
{
    /** \brief Will open the output stream in binary mode. It assumes you just want to ad some new stuff to the file immediately.*/
    if(output.is_open())//we don't want to leak handles or anything
    {
        output.close();
    }
...
/** \brief This method allows for a fast loading of file contents without keeping the file buffer open or an internal buffer copy.
 *  The file is opened in binary mode.
 * \param const char* location
 * \return STL string type with the raw file contents
 *
 */
{
    if(file.is_open())
...
{
    /** \brief Note: instanceIndex is the number of times a search word can be found in the target file. i.e. value = 1; value = 3;
...
/** \brief Force a file sync. Normally, a file buffer is used when accessing files since I/O writes can be expensive. This class further maintains its own
 *  copy of the buffer for editing prior to committing changes. This method not just transfers the contents to the standard file buffer, but it also
 *  attempts to execute a file sync so contents can be guaranteed to be written to the media and make the changes permanent.
 * \return nothing
 */
{
    FileClear();
...
{
    /** \brief Writes value into an specific 'tag' in file. i.e. value = 3; [search = value, value = 5] -> value = 5;*/
    if(buffer != "" && search != "")
    {
...
{
    /** \brief This methods find the instance of a keyword and modifies its value. i.e. value = 3;\nvalue = 4 [mod line 2 with 3] -> value = 3;\nvalue = 3.*/
...
{
    /** \brief Meant to be used with OpenFileForQuickWrite(). Unlike in the regular output mode, value will contain a manually
...
{
    /** \brief Searches for the search keyword until and increments the instance count (line count) each time it finds the search word.
...
{
    /** \brief Clears the file. WARNING: do not invoke this method if you are reading a file and the internal buffer does not have a copy of the file.
...
{
    /** \brief streamsToClose flags are all for all stream, in for input stream, and out for output stream. the flag is set to all by default.
...
{
    /** \brief This method should be called immediately after opening a file in write mode
...
{
    /** \brief This method should be called periodically after opening a file in read mode.
...
{
    /** \brief This method will erase all excess of newline and return characters!
...
{
    /** \brief The generated files receive an excess chunk after it is opened in write mode
...
void data_base::CreateNewFile(const char* location, size_t s)
{
    /** \brief This method empties the file if location = "" (you are assuming the file was already opened; see FileClear for
...
{
    /** \brief Static version of CreateNewFile
     **/
    std::ofstream out;
...
{
    /** \brief Constructor. This class will be used for reading files by default. However, changing the read flag to false will
...
data_base::~data_base()
{
    /** \brief Destructor. Makes sure the file handle is properly closed from the constructor event. It helps prevent file handle leaks.*/
    if(file.is_open())
    {
        file.close();
    }
...
{
    /** \brief This is my file copy function. It is a supporting function for the data_base class, since this class has the purpose
