...
size_t IOManager::RegisterBuffer(cstr file_path, bool inputMode)
{
    //Get a hash
    ...
    if(file_path)
    {
        ...
        {
            buffer.insert(id, new IONode(inputMode, id, file_path);
        }
    }
    //Unlock appropriate mutex
    owner_ref->UnlockMutex(mutex_io_id);
...
size_t IOManager::RegisterDataBase(cstr file)
{
    ...
    if(file)
    {
        ...
        {
            databases.insert(id, new IONode(true, id, file_path, true);
        }
    }
    //Unlock appropriate mutex
    owner_ref->UnlockMutex(mutex_db_id);
...
size_t IOManager::RegisterDataBase(DataBase& database)
{
    //Get a hash
    size_t id = Game::hasher();
    //Lock appropriate mutexes
